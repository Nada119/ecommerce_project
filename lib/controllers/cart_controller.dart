...
//product controller we call it
  void addItem(ProductModel product, int quantity) {
    ...
    if (_items.containsKey(product.id!)) {
      ...
      if (quantity > 0) {
        //gets for first time in the list
        _items.putIfAbsent(product.id!, () {
          return CartModel(
            id: product.id,
            name: product.name,
...
//only holds the number which on the cart no more data
  int get totalItems {
    var totalQuantity = 0;
    _items.forEach((key, value) {
      totalQuantity += value.quantity!;
...
//get all needed data for cart page, we won't return the map because we don't need the key only the list
  List<CartModel> get getItems {
    return _items.entries.map((e) {
      return e.value;
    }).toList();
